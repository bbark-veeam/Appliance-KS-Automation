...
- Added a second `--custom-post` starter template, **`example-custom-post-storage.sh`** — vmware-proxy iSCSI / NVMe-TCP / multipath connection prep (placeholder portals/IQNs/NQNs). Like the firewall template it's additive, unsupported, and a no-op until you edit it; it writes persistent config and/or a first-boot one-shot (mirroring the stock `start-iscsid-once.service`) rather than running live connection commands at install time.
