...
- Added a second `--custom-post` starter template, **`example-custom-post-storage.sh`** — vmware-proxy **Direct SAN access** prep (OS-level iSCSI / NVMe-TCP / multipath connectivity to the production LUNs; the case with no VBR-side config, unlike Backup from Storage Snapshots; placeholder portals/IQNs/NQNs). Like the firewall template it's additive, unsupported, and a no-op until you edit it; it writes persistent config and/or a first-boot one-shot (mirroring the stock `start-iscsid-once.service`) rather than running live connection commands at install time.
