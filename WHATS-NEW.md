...
- **Support for the Veeam 13.1+ appliance kickstart change — while keeping full 13.0 support.** From 13.1 onward the Infrastructure Appliance / JEOS ISO **consolidates all VIA roles into one install**: there is a single `proxy-ks.cfg`, and the host role is chosen at **first boot** via new `vbr_init.cfg` keys (`applianceRole.role` + `applianceRole.iSCSI`) instead of per-role kickstarts + role-named boot menus. The kit now:
  ...
  - For a **13.1+ VIA ISO**, builds the one JEOS install and writes your chosen role into the answer file (`applianceRole.*`) — no more relying on a per-role boot-menu entry that no longer exists.
- **New REQUIRED option for 13.1+ VIA builds: disk layout.** 13.1+ VIA ISOs offer a **Standard (multi-disk)** vs **Single-disk** deployment, and it must be chosen deliberately — so `build-appliance-iso.sh` / `make-golden-iso.sh` take **`--disk-layout standard|single`** (the GUI adds a matching **Disk layout** picker). It is **required** for a 13.1+ VIA build (no default) and **not applicable** to 13.0 VIA or any VSA build. VSA (`vsa`/`vbem`) is unchanged and never asks for it.
- **The `vmware-proxy` role is renamed to `storage-proxy`.** The iSCSI/NVMe-TCP storage prep isn't VMware-specific, so the clearer name is "backup proxy prepped for direct storage access (iSCSI/NVMe-TCP)". **`vmware-proxy` still works as a deprecated alias**, so existing scripts/commands keep running.
- **MFA rule corrected — it now matches the appliance instead of being stricter than it.** The previous kit *forced MFA on both accounts for the Hardened Repository role*, which was a simplification. The appliance's actual requirement applies to **the Hardened Repository role only**: an HR requires MFA on either a configured Security Officer **or** `veeamadmin`. So:
  - **`hardened-repo`:** disabling `veeamso` **forces `veeamadmin` MFA on** (the GUI locks the checkbox), because otherwise MFA would be nowhere and the appliance would reject the configuration.
  - **All other roles (`proxy`, `storage-proxy`, `vsa`, `vbem`):** no MFA requirement. You may disable `veeamso` **and** leave `veeamadmin` MFA off — exactly as the appliance's own setup wizard allows. The previous kit refused to build this, which blocked legitimate no-MFA deployments (for example a post-install script that authenticates to the VBR API, which cannot pass an MFA challenge).
  - `veeamso` MFA remains enforced whenever that account is enabled — that is an appliance rule, not a kit choice.
- **Heads-up for 13.1+ VIA builds: the boot menu no longer names the role.** On 13.0 the VIA boot menu showed role-named entries (`Veeam Hardened Repository`, `Veeam Infrastructure Appliance`, …). Because 13.1+ consolidates all VIA roles into one install, its boot menu shows the **disk topology** instead (`Standard (Multi-Disk) Deployment` / `Single-Disk Deployment`), so **all VIA roles boot an identical screen** and the role is not visible until after installation (Host Management reports it). Confirm which ISO you have from the **output file name**, which encodes both — e.g. `…_HARDENEDREPO_MULTIDISK_UNATTENDED.iso` vs `…_STORAGEPROXY_MULTIDISK_UNATTENDED.iso` — so avoid renaming built ISOs. Setting a per-role hostname prefix also makes the role obvious at first boot. (VSA ISOs are unaffected: `vsa`/`vbem` keep role-named boot entries.)
- The VIA ISO has shipped under two names (`VeeamInfrastructureAppliance*` and `VeeamJEOS*`); auto-detection of the source ISO matches **both**. The build model is detected from the ISO's structure, never its filename, so either name works.
- **What was validated on 13.1, stated plainly.** On the 13.1 GA ISOs (13.1.0.411), all three VIA roles were built **and booted** — `hardened-repo`, `storage-proxy` and `proxy` — covering both disk layouts, with the role confirmed in Host Management after first boot. On the VSA-family ISO, **`vbem` was built and booted**, and **`vsa` was built but NOT booted on 13.1**: the `vsa` role's disk requirement (480 GB) exceeded the free space in our test lab. The `vsa` role itself is boot-proven on a 13.0 ISO through this kit, and `vsa`/`vbem` share a single code path that the 13.1 work does not touch, so we consider the risk low — but it was not booted on 13.1 and we would rather say so than imply otherwise. If you are deploying `vsa` from a 13.1 ISO, treat your first build as a validation run.
