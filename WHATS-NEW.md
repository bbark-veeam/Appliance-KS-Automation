...
- **MFA rule corrected — no longer misleading.** The previous kit *forced MFA on both accounts for the Hardened Repository role*, which was a simplification. The real appliance rule is **MFA must be enabled on at least one account**. Now, for every role: if the Security Officer (`veeamso`) account is **enabled** it already carries MFA, so **veeamadmin MFA is your choice**; if `veeamso` is **disabled**, **veeamadmin MFA is enforced** (and, in the GUI, locked on) so MFA is never left off entirely. The build refuses an ISO that would have MFA nowhere.
- The VIA ISO's filename changed from `VeeamInfrastructureAppliance*` to `VeeamJEOS*` in 13.1; auto-detection of the source ISO now matches both.
